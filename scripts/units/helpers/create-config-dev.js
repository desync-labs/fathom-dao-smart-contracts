...
const configPath = path.join(configDir, 'config.dev.js')

if (fs.existsSync(configPath)) {
    console.log(`config.dev.js file already exists in ${configDir} directory!`);
}
else 
{
    ...
    fs.writeFile(configPath, constants, (err) => {
        if (err) throw err;
        console.log(`config.dev.js file has been created in ${configDir} directory!`);
    });
}
