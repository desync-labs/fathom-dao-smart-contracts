...
const configPath = path.join(configDir, 'config.qa.js')

if (fs.existsSync(configPath)) {
    console.log(`config.qa.js file already exists in ${configDir} directory!`);
}
else 
{
    ...
    fs.writeFile(configPath, constants, (err) => {
        if (err) throw err;
        console.log(`config.qa.js file has been created in ${configDir} directory!`);
    });
}
