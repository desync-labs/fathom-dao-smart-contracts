...
#### 1. [NEW] There's no `owners` array length validation in the constructor of `MultiSigWallet`[DONE]
##### Description
...
#### 2. [NEW] Removing owner without `revokeConfirmation` transaction in `MultiSigWallet`[NOTDONE]
##### Description
...
#### 3. [NEW] Changing the `timelock` address may cause re-execution of the proposals in `GovernorTimelockControl`[DONE]
##### Description
...
#### 4. [NEW] Transaction does not have a lifetime parameter in `MultiSigWallet`
##### Description[NOTDONE]
...
#### 5. [NEW] Governance can delete `TimelockAdmin` and the contract will lose its control in `TimelockController`[NOTDONE]
##### Description
...
#### 6. [NEW] There is no emergency shutdown mode in `Governor`[NOTDONE]
##### Description
...
#### 8. [NEW] There is no possibility to transfer standard `ERC20` tokens from the Governance balance in `MainTokenGovernor`[NOTDONE-CHECK]
##### Description
...
#### 9. [NEW] There is no option to migrate to another contract in the `VaultPackage` contract[DONE]
##### Description
...
#### 10. [NEW] There is a DoS possibility when calling `updateVault` in the `StakingHandlers` contract[NOTDONE]
##### Description
...
#### 11. [NEW] Tokens that get into the `VaultPackage` balance can be used to withdraw rewards in the contract `VaultPackage`[DONE-CHECK]
##### Description
...
####  12. [NEW] Calling `initializeStaking` in the `StakingHandlers` contract does not allocate rewards for `MAIN_STREAM` in `VaultPackage`[DONE]
##### Description
...
#### 14 .[NEW] `prohibitedEarlyWithdraw` is not set to `false` for `lockid` after unlocking in `StakingHandlers`
##### Description
...
#### 16. [NEW] No parameter check when adding transaction in `MultiSigWallet`[NOTDONE]
##### Description
...
#### [NEW] There is no time limit for executing proposal in `Governor`
##### Description
...
#### 17. [NEW] There is no check for the `msg.value` value available for execution in `Governor` and `TimelockController`
##### Description
...
#### 18. [NEW] There is no limit on the number of proposals for one proposer in `Governor`
##### Description
...
#### 19. [NEW] Possible overflow with calculations
##### Description
...
#### 20. [NEW] There's no logging of reverted transactions in `MultiSigWallet`
##### Description
